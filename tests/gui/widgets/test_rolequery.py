...
from setoolsgui.widgets.rolequery import RoleQueryTab


@pytest.fixture
def widget(mock_policy, request: pytest.FixtureRequest, qtbot: QtBot) -> RoleQueryTab:
    """Pytest fixture to set up the widget."""
    marker = request.node.get_closest_marker("obj_args")
    kwargs = marker.kwargs if marker else {}
    w = RoleQueryTab(mock_policy, **kwargs)
    qtbot.addWidget(w)
    w.show()
    return w


def test_docs(widget: RoleQueryTab) -> None:
...
def test_layout(widget: RoleQueryTab) -> None:
...
def test_criteria_mapping(widget: RoleQueryTab) -> None:
    """Test that widgets save to the correct query fields."""
    name, types = widget.criteria

    assert isinstance(widget.query, setools.RoleQuery)
