...
class MCPEncoder(json.JSONEncoder):
    ...
    def default(self, obj: Any) -> Any:
        ...
        if isinstance(obj, dta.DomainEntrypoint):
            return {
                "name": obj.name.name,
                "entrypoint_rules": sorted(obj.entrypoint),
                "execute_rules": sorted(obj.execute),
                "type_transition_rules": sorted(obj.type_transition),
            }
        ...
        if isinstance(obj, dta.DomainTransition):
            return {
                "source": obj.source.name,
                "target": obj.target.name,
                "transition_rules": sorted(obj.transition),
                "entrypoints": sorted(obj.entrypoints),
                "setexec_rules": sorted(obj.setexec),
                "dyntransition_rules": sorted(obj.dyntransition),
                "setcurrent_rules": sorted(obj.setcurrent),
            }
        ...
        if isinstance(obj, policyrep.ObjClass):
            d = {"name": obj.name, "statement": obj.statement()}
            try:
                d["common"] = obj.common.name
        ...
        if isinstance(obj, (policyrep.RoleAllow, policyrep.RoleTransition)):
            d = {
                "statement": obj.statement(),
                "ruletype": obj.ruletype.name,
                "source": obj.source.name,
